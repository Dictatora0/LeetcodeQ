...
----------------------
常见变体：
1. LeetCode 111 二叉树最小深度（叶子判定是关键）
2. LeetCode 110 平衡二叉树（深度 + 剪枝）
3. LeetCode 543 二叉树直径（由深度扩展）

面试追问：
Q1: DFS 和 BFS 都能做，如何选择？
A1: DFS 代码短；BFS 适合层序语义且可避免深递归栈。

Q2: 为什么空树深度是 0？
A2: 递归 base case，需要让父节点计算 `1 + max(left,right)` 正常成立。

Q3: 极端链式树会怎样？
A3: 递归栈深度变 O(n)，可改迭代避免栈溢出风险。
*/
