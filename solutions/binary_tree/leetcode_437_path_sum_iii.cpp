...
----------------------
常见变体：
1. LeetCode 112 路径总和（是否存在）
2. LeetCode 113 路径总和 II（列举路径）
3. LeetCode 666 路径总和 IV（编码树）

面试追问：
Q1: 为什么 prefixCount[0] 要初始化为 1？
A1: 支持“从根开始到当前节点正好等于 target”的路径统计。

Q2: 为什么回溯时要 prefixCount[cur]--？
A2: 前缀和映射只应对当前递归路径有效，离开节点要撤销状态。

Q3: 为什么优化解法是 O(n)？
A3: 每个节点仅做一次前缀和查询与更新，哈希操作均摊 O(1)。
*/
