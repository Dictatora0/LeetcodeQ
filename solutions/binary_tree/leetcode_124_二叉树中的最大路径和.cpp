...
----------------------
常见变体：
1. LeetCode 543 二叉树直径（边数版本）
2. LeetCode 687 最长同值路径
3. LeetCode 337 打家劫舍 III（树形 DP）

面试追问：
Q1: 为什么子树贡献要 `max(0, childGain)`？
A1: 负贡献会拉低路径和，最优路径不会主动接入负分支。

Q2: 为什么返回给父节点的是单边最大贡献？
A2: 父节点路径只能向下延伸一条边，双边只能用于当前节点封闭成答案。

Q3: 为什么全局答案必须是全局变量？
A3: 最优路径不一定经过根，需要在每个节点尝试更新并跨层保存。
*/
