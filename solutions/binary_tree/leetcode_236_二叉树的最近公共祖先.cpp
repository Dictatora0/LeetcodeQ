...
----------------------
常见变体：
1. LeetCode 235 BST 的最近公共祖先（利用有序性质）
2. LeetCode 1650 有父指针节点的最近公共祖先
3. LeetCode 1676 多节点最近公共祖先

面试追问：
Q1: 为什么遇到 root==p 或 root==q 可直接返回？
A1: 节点可作为自己的祖先，且向上回传可供父节点判断分叉。

Q2: 如果 p 或 q 不在树里怎么办？
A2: 本题保证存在；若不保证，需要额外记录是否都找到再决定返回。

Q3: 递归返回值语义是什么？
A3: 返回“当前子树中找到的 p/q 或其 LCA 节点”。
*/
