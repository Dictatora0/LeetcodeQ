...
----------------------
常见变体：
1. LeetCode 144 前序遍历（栈顺序调整）
2. LeetCode 145 后序遍历（双栈或反转技巧）
3. LeetCode 230 二叉搜索树中第 K 小元素（中序有序性质）

面试追问：
Q1: 为什么中序遍历在 BST 上有序？
A1: BST 定义是 left < root < right，中序顺序正好是 left-root-right。

Q2: 递归改迭代怎么保证顺序不乱？
A2: 一路压左子树，弹栈访问，再转向右子树，严格模拟系统栈。

Q3: 空间复杂度为什么是 O(h) 而不是 O(n)？
A3: 栈中最多同时保留从根到当前节点的一条路径，高度为 h。
*/
