 ...
 * 【30 秒讲清 DP】
 * 1. 先给判定依据：
 *    "题目有重叠子问题和最优子结构，适合动态规划。"
 * 2. 再给状态定义：
 *    "我定义 dp[...] 表示 ...，目标是求 dp[终点]。"
 * 3. 最后给转移：
 *    "当前状态由若干更小状态转移而来，遍历顺序按依赖关系确定。"
 *
 * 【面试官高频追问】
 * Q1: 为什么不是贪心？
 * A1: 当前最优选择不能保证全局最优，必须保留历史状态信息。
 *
 * Q2: 如何优化空间？
 * A2: 若 dp[i] 只依赖有限历史，可用滚动数组或变量替代。
 */
...
/*
 * 【常见错误 1：dp 含义不唯一】
 * ❌ 一会儿表示“前 i 个”，一会儿表示“到下标 i”
 * ✅ 开始前写清楚 dp 的语义并保持一致
 *
 * 【常见错误 2：初始化不符合语义】
 * ❌ 最小值问题却初始化为 0
 * ✅ 用 INF / -INF / base case 精确初始化
 *
 * 【常见错误 3：遍历顺序写反】
 * ❌ 0-1 背包容量正序，导致重复选同一物品
 * ✅ 0-1 背包倒序，完全背包正序
 *
 * 【常见错误 4：状态覆盖】
 * ❌ 空间压缩后提前覆盖尚未使用的状态
...
void testCoinChange() {
    Solution_CoinChange solver;
    vector<int> coins = {1, 2, 5};
    int amount = 11;
    cout << "凑成 " << amount << " 的最少硬币数: "
         << solver.coinChange(coins, amount) << endl;
}
