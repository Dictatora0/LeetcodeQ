...
| 容易混淆 | 关键区别问题 | 正确优先级 |
|---------|-------------|-----------|
| 滑动窗口 vs 前缀和 | 是否需要动态维护“合法窗口”约束？ | 需要收缩条件就选滑动窗口；只做区间和代数关系选前缀和 |
| DFS vs BFS | 是否要求最少步数/最短层数？ | 求最短步数优先 BFS；遍历所有路径优先 DFS |
| 二分查找 vs 双指针 | 是否存在单调性并且要 O(log n)？ | 有单调边界就二分；线性相向移动用双指针 |
| 单调栈 vs 堆 | 是否要“每个位置最近边界”？ | 最近更大/更小用单调栈；全局最值流式维护用堆 |
| 动态规划 vs 贪心 | 局部最优是否必然推出全局最优？ | 不能保证就用 DP，先写状态转移 |
| 回溯 vs DP | 是否需要“列出所有解”而非最值？ | 枚举所有解用回溯；最优值/计数优先 DP |

### 5. 30 秒口述模板（面试即用）

```text
1) 识别题型：
"题目是 [连续区间/最短路径/最优解]，对应 [窗口/BFS/DP]。"

2) 核心状态：
"我维护 [left,right / queue / dp[i][j]] 来表示 ...。"

3) 转移逻辑：
"每一步通过 [扩张收缩/出队入队/状态转移] 更新答案。"

4) 正确性抓手：
"因为 [首次到达最短/单调性/最优子结构]，算法正确。"
